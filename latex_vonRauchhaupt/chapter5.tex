...
\begin{table}[b!]
    ...
    The first machine serves as the \ac{DUT}. The second machine generates traffic to be sent to the \ac{DUT} via TRex.}\label{tab:specs}
    \centering
    \small
    \begin{tabular}{ll}
        \toprule
...
Since the goal of this study is to determine the most efficient \ac{IPC} type for Simplefail2ban, this abstraction does not diminish the findings of this thesis.
...
\begin{itemize}
    ...
    \end{itemize}
    \item Number of entries in \ac{eBPF} maps for IPv4 \& IPv6\@: 1000000
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Line count for the shared memory buffer segments\@: 1000000
...
\begin{itemize}
    ...
    \end{itemize}
    \item Differing \ac{IPC} type\@: DISK, SHM, SOCK
...
File logging is expected to perform worse than the other \ac{IPC} types discussed in this thesis.
...
\begin{figure}[h!]
    ...
    \caption[\ac{DUT} during socket measurements]{
        The graphic displays the data flow (left to right) on the \ac{DUT} when enabling sockets as the \ac{IPC} type.
        A packet can either be passed (XDP\_PASS) to the kernel or dropped (XDP\_DROP) before ever reaching it.}
...
Baseline measurements consist out of the experiments 1a, 1b and 2.
Meanwhile, the second reader experiments consist out of 3a and 3b.
...
\texttt{Log messages} represents the number or messages sent via the chosen \ac{IPC} type, while \texttt{Packets received by udp\_server} lists the number of packets reaching the application udp\_server.
...
Therefore, the number of packets that should be received deviates from the actual incoming traffic rates, resulting in slight inaccuracies when calculating the \texttt{relative drop}.
While not directly apparent at 100k \ac{PPS} due to slight rounding, this discrepancy can be detected by totalling \texttt{XDP\_DROP} and \texttt{XDP\_PASS} up and comparing it to the number of packets that should have theoretically been received.
Unfortunately, this issue is present in all data presented in this thesis.

Another notable thing is a stark difference in \ac{CPU} time, with a definitive spike when using the socket \ac{IPC} type.
...
File mode also outperforms shared memory and socket mode on \ac{CPU} usage, which was not expected.
...
\ac{IPC} types with lower latency and higher bandwidth can log more messages during the short influx of packets each ban cycle.
...
In contrast, both socket and shared memory seem to perform quite similar across all statistics except for \ac{CPU} time.
Here, the socket \ac{IPC} type occupies the \ac{CPU} almost ten times longer than shared memory.
...
When increasing invalid traffic rate to 1m \ac{PPS}, no significant changes to previous behavior can be observed.
...
Also, the latency of each \ac{IPC} type is clearly visible in the provided graph.
The shared memory \ac{IPC} type both starts and ends its ban cycles before the socket and file \ac{IPC} type.
Unexpectedly, the socket \ac{IPC} type still outperforms the file \ac{IPC} type at a minimum.
...
Its likely caused by the unbanning thread of Simplefail2ban having to handle 131068 clients, resulting in them being banned, on average, slightly longer than the intended 30 seconds.
...
Differences in \ac{CPU} time are prominent, with the socket \ac{IPC} performing worst and file \ac{IPC} performing best.
A general trend found in all measurements presented so far is the low throughput of the socket \ac{IPC} type.
...
Here, the shared memory and socket \ac{IPC} types outperform the measurement utilizing pure IPv6 displayed in \ref{fig:data:ipv6:30m:131068}.
...
Now, the socket \ac{IPC} type is unable to defend against the \ac{DoS} attack and the system is overwhelmed.
...
The socket \ac{IPC} type can not abandon slow readers, it has to wait for each reader to receive all data.
