...
#include <stdlib.h>
#include <string.h>
#include "include/atomic_hashtable.h"

/***
* Author: taymindis@gmail.com - taymindis
* Atomic Hashtable, The faster free and writeable, travese and read without locking
...
****/
void* __atomic_hash_alloc_slot_(__atomic_hash *atom_hash, unsigned long hash_index);
int __atomic_hash_release_slot_(__atomic_hash *atom_hash, struct __atomic_node_s *n);

int __atomic_hash_realloc_buffer_(__atomic_hash *atom_hash);

// Logic Derived from djb2 by Dan Bernstein.
...
void*
__atomic_hash_alloc_slot_(__atomic_hash *atom_hash, unsigned long hash_index) {
    // printf("hash index is %lu\n", hash_index);
    struct __atomic_node_s * return_ = atom_hash->node_buf + hash_index;
...
int
__atomic_hash_release_slot_(__atomic_hash *atom_hash, struct __atomic_node_s *n) {
    // HashKey temp_key = n->key;
    if (n->key) {
        __atomic_hash_free_fn(n->key);
        n->key = NULL;
    }
...
/*API Endpoint*/
__atomic_hash*
__atomic_hash_init(size_t hash_size, atomic_hash_read_node_fn read_node_fn_, atomic_hash_free_node_fn free_node_fn_) {
    if (hash_size <= 1) {
    ...
    if (read_node_fn_ == NULL)
        printf("%s\n", "WARNING:: Must have read node function provided to read the node, else do not use read function");
    ...
    __atomic_hash *atom_hash = __atomic_hash_malloc_fn(sizeof(__atomic_hash));
    ...
    if (atom_hash->node_buf == NULL) {
        __atomic_hash_free_fn(atom_hash);
        return 0;
    }
...
int
__atomic_hash_realloc_buffer_(__atomic_hash *atom_hash) {
...
__atomic_hash_check_add_slot_(__atomic_hash *atom_hash) {
    if (__atomic_fetch_add(&atom_hash->size, 1, __ATOMIC_ACQUIRE) >= atom_hash->total_size) {
        ...
        if (__atomic_load_n(&atom_hash->size, __ATOMIC_SEQ_CST) > atom_hash->total_size) {
            __atomic_hash_realloc_buffer_(atom_hash);
        }
...
int
__atomic_hash_put(__atomic_hash *atom_hash, HashKey key_, void *value) {
    int success = 0;
    ...
    struct __atomic_node_s *node = __atomic_hash_alloc_slot_(atom_hash, get_hash_index(key_));

    if (node) {
        node->val = value;
        node->key = __atomic_hash_malloc_fn(size_of_key_with_padding);
        memcpy(node->key, key_, size_of_key_with_padding);
        node->key[keyLen] = 0;
        success = 1;
    } else {
...
SUCCESS:
    ...
    if (node) {
        ...
        if (buffer->key && strncmp(key_, buffer->key,  keyLen) == 0) {
            node->val = value;
            node->key = __atomic_hash_malloc_fn(size_of_key_with_padding);
            memcpy(node->key, key_, size_of_key_with_padding);
            node->key[keyLen] = 0;
            // node->key_len = buffer->key_len;
            __atomic_hash_release_slot_(atom_hash, buffer); // release the old slot
        } else {
            // if someone has replace this slot, release back the
            __atomic_hash_release_slot_(atom_hash, node);
            found = NULL;
        }
...
SUCCESS:
    ...
    if (buffer->key &&
            strncmp(key_, buffer->key,  keyLen) == 0 &&
            __atomic_hash_release_slot_(atom_hash, buffer)) {
...
__atomic_hash_destroy(__atomic_hash *atom_hash) {
    ...
    __atomic_hash_free_fn(atom_hash->node_buf);
