...
#define DEFAULT_LOG "simplelogstash_4socks.log"
...
int write_routine(void){
    ...
    struct sock_reader_arg_t * sock_arg = NULL;
    ...
    if((sock_arg = (struct sock_reader_arg_t *) calloc(sizeof(struct sock_reader_arg_t), 1)) == NULL)
    {
        perror("calloc failed");
        exit(EXIT_FAILURE);
    }

    if((retval = sock_init((union sock_arg_t *) sock_arg, SOCK_READER)) != IO_IPC_SUCCESS)
    {
        if(retval > 0) {perror("sock_init failed");}
        else { fprintf(stderr, "sock_init failed with error code %d\n", retval);}
    ...
    while (running)
    {
       ...
       if(second_buf)
       {
            if((read_index = sock_readv(sock_arg, io_buf2->iovs)) < 0)
            {
                fprintf(stderr, "Error in sock_readv : error code %d\n", read_index);
                error = true;
                break;
            }
       }
       ...
       {
            if((read_index = sock_readv(sock_arg, io_buf1->iovs)) < 0)
            {
                fprintf(stderr, "Error in sock_readv : error code %d\n", read_index);
                error = true;
                break;
    ...
    if((retval = sock_finalize((union sock_arg_t *)&sock_arg, SHMRBUF_READER)) != IO_IPC_SUCCESS)
    {
        fprintf(stderr,"sock_finalize failed with error code %d\n", retval);
        error = true;
    }
